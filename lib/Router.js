 ...
 *
 */
var Router = function(moduleName, modulePath, modulePlugin) {
        ...
        function matchRoutes() {
          ...
          for (var i = 0, l = routes.length; i < l; i++) {
            ...
            if (captures && (!routeMethod || req.method === routeMethod)) {
              ...
              if (!router.modulePlugin) {
                calipso.silly('-- NO PLUGIN loaded for ', router.moduleName)
              } else {
                calipso.silly('++ plugin loaded for ', router.moduleName);
              }
              ...
              // Run our module fn
              handlePlugin(requestUrl.pathname, router.modulePlugin, route, function (scope) {

                var hasPrePlugin = calipso.utils.hasProperty('plugin.pre', scope),
                  hasPostPlugin = calipso.utils.hasProperty('plugin.post', scope);
                
                if (hasPrePlugin) {
                    scope.plugin.pre(req, res, template, block, function (preReq, preRes, preTemplate, preBlock, preProcess) {
                      scope.fn(preReq, preRes, preTemplate, preBlock, group(), preProcess);
                    });
                } else {
                    scope.fn(req, res, template, block, group());
                }
              })
...
function handlePlugin(request, plugin, scope, next) {
  if (scope.path) {
    if (typeof scope.path.split === 'function') {
      // 'GET /r/o/u/t/e
      var method = scope.path.split(' ').pop();
      if (method === request) {
        scope.plugin = plugin[method];
      }
    }
  }
  next(scope);
