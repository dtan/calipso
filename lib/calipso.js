...
function loadModules(options) {
  ...
  calipso.lib.fs.readdirSync(__dirname + '/../modules').forEach(function(type){
    ...
    if(type != "README" && type != '.DS_Store' && type !== 'plugins') {  // Ignore the readme file and .DS_Store file for Macs
      ...
      calipso.lib.fs.readdirSync(__dirname + '/../modules/' + type).forEach(function(name){
        ...
        if(name != "README" && name != '.DS_Store') { // Ignore the readme file and .DS_Store file for Macs
              ...
              var pluginPath = 'modules/plugins/' + name + '/' + name + '.plugin';
...
function requireModule(module, modulePath, reload) {
  ...
  var modulePlugin = module.plugin;
  ...
  try {
    ...
    try {
      var pluginFn = require(modulePlugin);
      // Attach a router
      module.router = require('./Router').Router(module.name, modulePath, pluginFn);
    } catch (ex) {
      // Attach a router
      // no plugin, but still make sure to continue
      module.router = require('./Router').Router(module.name, modulePath, null);
    }
