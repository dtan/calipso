...
calipso = exports = module.exports = {
  ...
  calipsoRouter: function(appReference, config, initCallback) {
    ...
    console.log("appReference: ", appReference.stack)
    ...
    calipso.theme.render(req, res, function(err, content) {
      ...
      if(err) {
        ...
        res.end("<html><h2>A fatal error occurred!</h2>" +
                "<p>" + (err.xMessage ? err.xMessage : err.message ? err.mssage : err) + "</p>" +
 ...
 * Configure a theme using the theme library.
 */
function configureThemeAdmin(next, overrideTheme) {

  var themeName = overrideTheme ? overrideTheme : calipso.app.set('config').themeAdmin;
  require('./Theme').Theme(themeName, function(theme) {

    calipso.themeAdmin = theme;
    if (!calipso.theme) {
      var dt = calipso.defaultThemeAdmin;
      calipso.error('THE `' + themeName + '` ADMIN THEME IS MISSING, USING DEFAULT ADMIN THEME `' + dt + '`')
      configureTheme(next, dt);
    } else {
      // Search for middleware that already has themeStatic tag
      calipso.app.stack.forEach(function(middleware,key) {
       ...
       if(middleware.handle.tag === 'themeAdminStatic') {
          // Replace it
          var oneDay = 86400000;
          var themeStatic = calipso.lib.express.static(calipso.app.path + '/themes/' + themeName + '/public',{maxAge:oneDay});        
          themeStatic.tag = 'themeAdminStatic';
          calipso.app.stack[key].handle = themeStatic;
       }
      });
